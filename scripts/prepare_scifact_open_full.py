...
from src.data.load_scifact_open import (
    ...
    load_scifact_open_full_corpus,
    normalize_scifact_open_full_documents,
...
def main():
    ...
    corpus_path = Path("data/raw/scifact_open/full_corpus/corpus.jsonl")
    ...
    logger.info("Loading full SciFact-Open raw data...")
    raw_claims = load_scifact_open_claims(claims_path)
    raw_docs = load_scifact_open_full_corpus(corpus_path)

    logger.info("Normalizing full-corpus documents...")
    docs = normalize_scifact_open_full_documents(raw_docs)
    ...
    logger.info("Chunking full corpus...")
    chunks = chunk_documents(docs, max_tokens=220, overlap_tokens=40)
    validate_unique_ids(chunks, "chunk_id")
    logger.info(f"Chunks: {len(chunks)}")

    docs_out = docs_out_dir / "scifact_open_full_documents.jsonl"
    queries_out = queries_out_dir / "scifact_open_full.jsonl"
    chunks_out = chunks_out_dir / "scifact_open_full_chunks.jsonl"
